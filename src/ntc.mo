...
actor class NTCminter() = this {
    ...
    let NTC_ledger = ICRCLedger.Ledger<system>(NTC_mem_v1, NTC_ledger_id, #id(0), Principal.fromActor(this));
    ...
    // Keep old stable - need to delete
    stable let NTC2Can_requests = BTree.init<Nat64, NTC2Can_request>(?32); 
    ...
    private func subaccount2canister(subaccount : [Nat8]) : Principal {
        ...
        Principal.fromBlob(Blob.fromArray(Iter.toArray(Array.slice(subaccount, 31 - size:Nat, 31))));
    };
    ...
    public query func get_account(canister_id : Principal) : async (Account.Account, Text, Principal) {
        ...
        (
            acc,
            Account.toText(acc),
            subaccount2canister(Blob.toArray(canister2subaccount(canister_id)))
        );
    }
